...
public abstract class Person implements java.io.Serializable {
	...
	public void setDOB(Date DOB) throws PersonException {
		...
		if (CurrentYear.get(Calendar.YEAR) - Year > 100) {
	...
	public void setPhone(String number_new) throws PersonException {
		
		String regex = "^\\({1}([0-9]{3})\\){1}-{1}([0-9]{3})-{1}([0-9]{4})$";
		Pattern pattern = Pattern.compile(regex);
		Matcher matcher = pattern.matcher(number_new);
		
		if (matcher.matches() == true) {
			
			this.phone_number = number_new;
		}
		else {
			throw new PersonException(this);
		}
